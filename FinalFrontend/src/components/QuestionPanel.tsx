...
const QuestionPanel: React.FC<QuestionPanelProps> = ({ question, onAnswer }) => {
    return (
        <div className="bg-[#3a3a3a] rounded-lg p-4 sm:p-6 pixel-border">
            <h2 className="text-base sm:text-xl font-pixel text-[#ffd700] mb-4 sm:mb-6 text-center">
                {question.text}
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-4">
                {question.options.map((option, index) => (
                    <motion.button
                        ...
                        whileTap={{ scale: 0.98 }}
                        onClick={() => onAnswer(option)}
                        className="p-2 sm:p-4 text-sm sm:text-lg font-pixel bg-[#2b2b2b] text-white rounded-lg
                     hover:bg-[#ffd700] hover:text-[#2b2b2b] transition-colors
                     focus:outline-none border-b-2 sm:border-b-4 border-[#1a1a1a] hover:border-[#cc9900]
                     active:border-b-0 active:mt-0.5 sm:active:mt-1"
                    >
                        {option}
                    </motion.button>
                ))}
